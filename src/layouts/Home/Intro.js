    ...
    >
      <Transition in key={theme.themeId} timeout={3000}>
        {(visible, status) => (
          <Fragment >
            <DisplacementSphere />
            <header className={styles.text}  style={{height:'60%'}}>
              <h1 className={styles.name} data-visible={visible} id={titleId}>
                <DecoderText text="Robinson Roa" delay={300} />
              </h1>
              <Heading level={0} as="h2" className={styles.title}>
                ...
                <div className={styles.row} component="span">
                  <AnimatePresence>
                  <span className={styles.disciplines} style={(theme.themeId == 'light' ? {color:'#444444'}: {color:''})} >
                      {currentDiscipline}
                  </span>
                  </AnimatePresence>
                </div>
